...
//                this.workspaceKey);
    ...
    public void report(SortedMap<String, Gauge> gauges,
        ...
        Instant timeGenerated = Instant.now();
        for (Map.Entry<String, Gauge> entry : gauges.entrySet()) {
            if (entry.getValue().getValue() != null) {
                //nodes.add(addProperties(entry.getKey(), entry.getValue()));
                nodes.add(addProperties(entry, timeGenerated));
            }
        }
        ...
        try {
            ...
            // We need to send each individual message so the batching can take over.
            for (JsonNode node : nodes) {
                System.out.println(this.mapper.writeValueAsString(node));
            }
    ...
    private JsonNode addProperties(Map.Entry<String, ? extends Metric> entry, Instant timeGenerated) {
        ...
        node.put("time_generated", timeGenerated.toString());
        return node;
    }
